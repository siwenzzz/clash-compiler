...
instance Backend backend => HasIdentifierSet (BlockState backend) where
  ...
  identifierSet f bs =
    fmap
      (BlockState (_bsDeclarations bs) (_bsHigherOrderCalls bs))
      (identifierSet f (_bsBackend bs))
...
newName nm = zoom bsBackend $ Id.makeBasic nm
...
outputFn fromTypes toType exprFn inNames0 (TExpr outType (Identifier outName Nothing))
  | outType == toType = do
      inNames1 <- mapM newName inNames0
...
instHO bbCtx fPos (resTy, bbResTy) argsWithTypes = do
  ...
  args1 <- mapM ( fmap pure
                . fmap Text
                . fmap Id.toLazyText
                . Id.make
                . argName ) [0..length args0 - 1]
  let
  ...
  let component = Component (Decl fPos fSubPos (res:zip args1 argTypes))
  rendered0 <-
