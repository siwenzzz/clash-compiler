...
import           Control.Monad                 (when, replicateM, zipWithM, join)
...
mkBlackBoxContext bbName resId args@(lefts -> termArgs) = do
    -- Make context inputs
    -- 'unsafeMake' is safe here: 'mkUniqueArguments' made sure these names
    -- are unique.
    let resNm = id2identifier resId
    ...
    ctxName1 <- fromMaybe (Id.toText resNm) <$> Lens.view setName
...
prepareBlackBox _pNm templ bbCtx =
  case verifyBlackBoxContext bbCtx templ of
    Nothing -> do
      (t2,decls) <-
        onBlackBox
          (fmap (first BBTemplate) . setSym Id.makeAs bbCtx)
...
mkArgument bbName bndr nArg e = do
    ...
    ((e',t,l),d) <- case hwTyM of
      ...
      Just hwTy -> case collectArgsTicks e of
        (C.Var v,[],_) -> do
          -- Safe to use 'unsafeMake' here. 'mkUniqueNormalized' should have made
          -- it unique.
...
mkFunInput resId e =
 ...
 in  withTicks ticks $ \tickDecls -> do
  ...
  templ <- case appE of
            ...
            C.Var fun -> do
              topAnns <- Lens.use componentNames
              case join (snd <$> lookupVarEnv fun topAnns) of
                Just _ ->
      ...
      if null tyArgs
        then
          withTicks ticks $ \tickDecls -> do
            appDecls <- mkFunApp (Id.unsafeMake "~RESULT") fun tmArgs tickDecls
