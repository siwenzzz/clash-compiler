...
data NetlistState
  ...
  , _componentNames :: VarEnv (Identifier, Maybe (Maybe (TopEntity)))
  -- ^ Names of components (to be) generated during this netlist run.
...
emptyBBContext name
  = Context
  { bbName        = name
  , bbResult      = (Identifier resultId Nothing, Void Nothing)
  , bbInputs      = []
  , bbFunctions   = empty
  , bbQsysIncName = []
  , bbLevel       = (-1)
 ...
 where
  resultId = UniqueIdentifier "__EMPTY__" "__EMPTY__" [] Basic VHDL
