...
import           Clash.Core.Type
  (Type (..), coreView1, splitFunForallTy)
...
genNetlist
  ...
  -> BindingMap
  -- ^ Global binders
  -> VarEnv (Identifier, Maybe (Maybe TopEntity))
  -- ^ Top entities names
  -> CompiledPrimMap
...
genNetlist isTb opts reprs globals tops primMap tcm typeTrans iw ite be seen env prefixM topEntity = do
...
runNetlistMonad
  ...
  -- ^ Action to run
  -> IO (a, NetlistState)
runNetlistMonad isTb opts reprs s tops p tcm typeTrans iw ite be seenIds0 env prefixM
  ...
  where
    ...
    (componentNames_, seenIds1) =
      genNames (opt_newInlineStrat opts) prefixM seenIds0 tops s
...
genNames
  ...
  -- ^ Pre-generated names
  -> BindingMap
  -> (VarEnv (Identifier, Maybe (Maybe TopEntity)), IdentifierSet)
 ...
 where
  go env_ (bindingId -> id_) =
    case lookupVarEnv id_ env_ of
      Just _ -> pure env_
      Nothing -> do
        nm <- Id.make (genComponentName newInlineStrat prefixM id_)
        pure (extendVarEnv id_ (nm, Nothing) env_)
...
genTopNames prefixM esc hdl tops =
 ...
 where
  ...
  goFixed env (topId, ann) = do
    ...
    pure (extendVarEnv topId (topNm, Just (Just ann)) env)

  goNonFixed env id_ = do
    topNm <- Id.make (genComponentName True prefixM id_)
    pure (extendVarEnv id_ (topNm, Just Nothing) env)
...
genComponentT compName0 componentExpr = do
  (compName1, topEntMM) <- (`lookupVarEnv'` compName0) <$> Lens.use componentNames
...
--          ((args, binds, res{varType=varType compName0}))
          ((args, binds, res))
      Left err ->
...
mkFunApp dstId fun args tickDecls = do
  topAnnMM <- fmap snd . lookupVarEnv' <$> Lens.use componentNames <*> pure fun
  tcm      <- Lens.use tcCache
  case (isGlobalId fun, topAnnMM) of
    (True, Just annM)
      | let ty = varType fun
