...
infoBitVector f e p args
 ...
 where
  ...
  go Proxy x = do
    a <- fromValueForce @(BitVector m) e x
    resTy <- resultType p args
    toValue (f a) resTy
...
primConcat e p args
 ...
 where
  ...
  go Proxy Proxy x y = do
    a <- fromValueForce @(BitVector n) e x
    b <- fromValueForce @(BitVector m) e y
    resTy <- resultType p args
...
primSetSlice e p args
 ...
 where
  ...
  go pM pI pN x y = do
    a <- fromValueForce @(BitVector (m + 1 + i)) e x
    b <- fromValueForce @(BitVector (m + 1 - n)) e y
    let sM   = snatProxy pM
        sN   = snatProxy pN
...
primSplit e p args
 ...
 where
  ...
  go Proxy Proxy x = do
    a <- fromValueForce @(BitVector (m + n)) e x
    resTy <- resultType p args
...
extendingNumABitVector f e p args
 ...
 where
  ...
  go Proxy Proxy x y = do
    a <- fromValueForce @(BitVector m) e x
    b <- fromValueForce @(BitVector n) e y
    resTy <- resultType p args
