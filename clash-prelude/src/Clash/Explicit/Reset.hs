...
resetGlitchFilter SNat clk rst =
  unsafeFromHighPolarity (mealy clk noReset enableGen go Idle shiftReg)
 where
  shiftReg =
    register
      clk noReset enableGen
      noGlitch
      (liftA2 (<<+) shiftReg (unsafeToHighPolarity rst))
  ...
  go gfs sreg
    | sreg == noGlitch = (InReset, True)
    | Idle <- gfs = (Idle, True)
    | otherwise = (if V.head sreg then InReset else Idle, V.head sreg)

  noGlitch :: Vec glitchlessPeriod Bool
  noGlitch = V.repeat True
